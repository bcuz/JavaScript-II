...
// they just want an arr of first names capitalized here?
...
// need an initial value, otherwise the initial value becomes the first object, and each donation is tacked on to it, which is odd.
let ticketPriceTotal = runners.reduce((total, person) => total + person.donation, 0);
console.log(ticketPriceTotal);

// check work w/ for loop
// let total = 0
// for (const person of runners) {
...
let avg = runners.reduce((total, person) => total + person.donation, 0) / runners.length;
console.log(avg);

// Problem 2
// get all emails
let emails = runners.map(person => person.email);
console.log(emails);

// Problem 3
// how many ppl work at Quaxo
let quaxo = runners.filter(person => person.company_name == 'Quaxo').length;
console.log(quaxo);
